...
#define INI_KEY "user_code"
#define INI_KEY_DEFAULT "zgmicro"
...
void now_time(void)
{
    ...
    value= rand();
    printf("%02d:%02d:%02d\r\n",h,m,s); // %02d是将数字按宽度为2，采用右对齐方式输出，若数据位数不到2位，则左边补0 \r到当前行最左
}

int main()
{
    int input;
    //system("type D:\\4_ZGW\\VimcBT\\VA9638B\\Src_Mod38\\branches\\VA9638B_V8000_PowerManage\\corebt\\va9638b_bt.uvproj.c"); // 命令字符串,运行DOS命令TYPE
    printf("I am running EXE management now!\r\n");
    char bt[256]="start \"\" \"D:\\4_ZGW\\VimcBT\\VA9638B\\Src_Mod38\\branches\\VA9638B_V8000_PowerManage\\corebt\\va9638b_bt.uvproj\""; // 已给定命令内容的字符变量
    char top[256]="start \"\" \"D:\\4_ZGW\\VimcBT\\VA9638B\\Src_Mod38\\branches\\VA9638B_V8000_PowerManage\\coretop\\va9638b_top.uvproj\""; // 已给定命令内容的字符变量

    char mpflash[256]="start \"\" \" F:\\8.ZGW\\4_VA9638项目\\18.三核代码发布\\SDK\\VA9638B_SDK\\Beta\\SDK\\V8008\\Tools\\MPFlashTool2.0\\MPFlashTool2.0.exe\"";
    char vs_code[256]="start \"\" \"D:\\2_InstalPackage\\9_VisualStudioCode\\Microsoft VS Code\\Code.exe\"";
   ...
   // system(st); // 运行外部a.exe文件
    ...
    while(1)
    {
        ...
        switch (intput){
            ...
            case 3: system(vs_code);
            break;

            case 4: system(mpflash);
            break;
...
//system("pause");
	...
	CShdIni _ini;
	_ini.SetFileName ( "encrypt.ini" );
	CString _cS = _ini.GetString ( INI_KEY, INI_KEY_DEFAULT );
	_ini.WriteString ( INI_KEY, _cS );
